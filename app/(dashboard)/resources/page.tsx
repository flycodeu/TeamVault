...
const kinds = [["", "全部场景"], ["PROJECT", "项目"], ["TOOL", "工具 / 系统"], ["KNOWLEDGE", "知识 / 文档"], ["PERSONAL", "个人"], ["OTHER", "其他"]]
...
export default async function ResourcesPage({ searchParams }: { searchParams: Promise<{ q?: string; kind?: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-7 md:px-8 md:py-9">
      <div className="flex items-center justify-between gap-4"><div><h1 className="text-2xl font-semibold">模块</h1><p className="mt-1 text-sm text-muted-foreground">项目、工具和个人资料都从一个模块开始。</p></div><Button asChild><Link href="/resources/new"><Plus />新建模块</Link></Button></div>
