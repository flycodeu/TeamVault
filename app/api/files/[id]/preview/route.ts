...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try { const body = await fs.readFile(target); return new Response(body, { headers: { "Content-Type": file.previewStatus === "SUCCESS" ? "application/pdf" : file.mimeType, "Content-Disposition": `inline; filename="${encodeURIComponent(file.originalName)}"` } }) } catch { return NextResponse.json({ success: false, error: "预览不存在" }, { status: 404 }) }
}
