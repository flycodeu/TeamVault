...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!file || !(await canViewFile(file.resourceId)) || !file.mimeType.startsWith("image/")) return NextResponse.json({ success: false, error: "无可用缩略图" }, { status: 404 })
  const thumbnailRoot = path.resolve(process.cwd(), "data", "thumbnails")
