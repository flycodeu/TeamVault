...
> 在线预览采用浏览器端原生只读方案，不依赖 LibreOffice、FFmpeg 或任何外部第三方转换服务，极速轻量；超限文件（如 PPTX > 50MB）会明确降级为下载。视频支持浏览器可直接播放的标准格式（MP4、WebM 等）；不兼容的封装格式可一键下载后使用本地播放器（如 VLC / PotPlayer 等）查看。
