...
async function main() {
  ...
  const target = path.resolve(process.cwd(), "data", "backups", `teamvault-${stamp}`)
  await fs.mkdir(target, { recursive: true })
  const backupDb = path.join(target, "teamvault.db")
  await sqlite.backup(backupDb)
  await copyTree(path.resolve(process.cwd(), "data", "files"), path.join(target, "files"))
  await copyTree(path.resolve(process.cwd(), "data", "previews"), path.join(target, "previews"))
